...
formatted_timestamp_commas = locale.format_string(
    "%.4f", unix_timestamp_float, grouping=True)
...
if 1 <= day_number <= 9:
    formatted_date_final = formatted_date_with_padded_day.replace(
        f" {day_number:02d}",  f" {day_number}")
else:
    formatted_date_final = formatted_date_with_padded_day

print(
    f"Seconds since January 1, 1970: {formatted_timestamp_commas} or {formatted_timestamp_scientific} in scientific notation")
